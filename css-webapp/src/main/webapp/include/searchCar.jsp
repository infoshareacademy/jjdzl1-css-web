...
<nav>
    <ul class="pagination">
        <c:if test="${currentPage > 1}">
            <li class="page-item"><a class="page-link" href="searchCar?make=${make}&model=${model}&fuel=${fuel}&currentPage=${currentPage-1}">Previous</a>
            </li>
        </c:if>
        ...
        <c:forEach begin="1" end="${noOfPages}" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item"><a class="page-link" href="searchCar?make=${make}&model=${model}&fuel=${fuel}&currentPage=${i}">${i}</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        ...
        <c:if test="${currentPage lt noOfPages}">
            <li class="page-item"><a class="page-link" href="searchCar?make=${make}&model=${model}&fuel=${fuel}&currentPage=${currentPage+1}">Next</a>
            </li>
        </c:if>
    </ul>
</nav>
